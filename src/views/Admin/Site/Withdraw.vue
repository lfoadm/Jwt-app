...
<template>
  <VContainer class="w-20">
    ...
    <h1>Solicitar resgate de dinheiro! Seu saldo atual é: <strong class="text-blue">R$ {{ useAuthAmountStore.balance.amount
    }}</strong></h1>
    <hr class="my-10">
          ...
          <v-row class="d-flex mb-3">
            <v-row dense>
              <v-col cols="12" class="mt-4">
                <v-text-field label="Informe o valor do saque..." variant="outlined" prepend-icon="mdi-account-cash"
                  id="body" required class="mx-10" v-model="value">
                </v-text-field>
              </v-col>
            </v-row>
            <v-col cols="12">
              <v-btn type="submit" class="text-white" color="secondary" size="large" block flat :loading="isSubmitting"
                :disabled="isSubmitting">Solicitar saque</v-btn>
            </v-col>
          </v-row>
        </form>
